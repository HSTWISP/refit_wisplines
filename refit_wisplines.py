...
def trim_spec(tbdata_blue, tbdata_red, config_pars):
    ...
    return [lam_spec, flux_spec, error_spec, contam_spec] 
...
def emissionline_model(pars, x): 
    ...
    #### pegged parameters: 
    nnodes = pars[0]    #### calculated from len(node_wave) and passed here. 
...
def fit_obj(input_list):   
    ...
    dz = 0.02      ### this is the maximum shift from the line list redshift/best estimate.  
...
def loop_field(dirtospectra, par, linelist, outlist, configfile): 
    
    linetable = asciitable.read(linelist)
    config = read_config(configfile)
     
    #### grab the items that are needed to do the fitting. 
    beams = linetable['NUMBER']
    redshifts = linetable['redshift'] 
    fwhm = linetable['RestFrameFWHM'] ### want observed
    wavelength = linetable['Wavelength'] 
    linesnr = linetable['SNR']  
  
    ### grab the rest of the items that are getting carried from the old to new list 
    col3 = linetable['NumberofUserIdentifications'] 
    col4 = linetable['RA'] 
    col5 = linetable['Dec'] 
    col6 = linetable['Jmagnitude'] 
    col7 = linetable['Hmagnitude'] 
    col8 = linetable['A_IMAGE'] 
    col9  = linetable['B_IMAGE'] 
    #col35 = linetable['FWHMFlag'] 
    #col36 = linetable['RedshiftFlag'] 
    #col37 = linetable['ContaminationFlag'] 
    #col38 = linetable['EdgeFlag'] 
    #col39 = linetable['QualityFlag']
    
  
    #### find a unique list of objects instead of a list of lines. 
    beams_unique, indices = np.unique(beams, return_index = True) 
    fwhm_unique = fwhm[indices] 
    redshifts_unique = redshifts[indices]  
    wave_unique = wavelength[indices] 
    figstring = ' '

    outfile_diagnostic = open('Par' + str(par) + '_fwhm_z.dat', 'w')
    outfile_diagnostic.write('Number    fwhm_guess    z_obj\n')
 
    outfile  = open(outlist, 'w')
    outfile.write('#1 ParID \n') 
    outfile.write('#2 NUMBER \n') 
    outfile.write('#3 NumberofUserIdentifications \n') 
    outfile.write('#4 RA \n') 
    outfile.write('#5 Dec \n') 
    outfile.write('#6 Jmagnitude [99.0 denotes no detection] \n') 
    outfile.write('#7 Hmagnitude [99.0 denotes no detection] \n') 
    outfile.write('#8 A_IMAGE \n') 
    outfile.write('#9 B_IMAGE \n') 
    outfile.write('#10 redshift \n') 
    outfile.write('#11 redshift_err \n') 
    outfile.write('#12 dz_oiii \n') 
    outfile.write('#13 dz_oii \n') 
    outfile.write('#14 dz_siii_he1 \n') 
    outfile.write('#15 G141_FWHM_Obs  [Angs] \n') 
    outfile.write('#16 G141_FWHM_Obs_err  \n')  
    outfile.write('#17 oii_flux \n') 
    outfile.write('#18 oii_error \n')
    outfile.write('#19 oii_EW_obs \n')  
    outfile.write('#20 hg_flux \n') 
    outfile.write('#21 hg_err \n')  
    outfile.write('#22 hg_EW_obs \n') 
    outfile.write('#23 hb_flux \n') 
    outfile.write('#24 hb_err \n')  
    outfile.write('#25 hb_EW_obs \n') 
    outfile.write('#26 oiii_flux [both lines] \n') 
    outfile.write('#27 oiii_err [both lines] \n')  
    outfile.write('#28 oiii_EW_obs [both lines] \n') 
    outfile.write('#29 hanii_flux \n') 
    outfile.write('#30 hanii_err \n') 
    outfile.write('#31 hanii_EW_obs \n')  
    outfile.write('#32 sii_flux \n') 
    outfile.write('#33 sii_err \n') 
    outfile.write('#34 sii_EW_obs \n')  
    outfile.write('#35 siii_9069_flux \n') 
    outfile.write('#36 siii_9069_err \n') 
    outfile.write('#37 siii_9069_EW_obs \n') 
    outfile.write('#38 siii_9532_flux \n') 
    outfile.write('#39 siii_9532_err \n')  
    outfile.write('#40 siii_9532_EW_obs \n') 
    outfile.write('#41 he1_10830_flux \n') 
    outfile.write('#42 he1_10830_err \n') 
    outfile.write('#43 he1_10830_EW_obs \n') 
    #outfile.write('#35 oldFWHMFlag [1: Deviation by more than factor of 1.5] \n') 
    #outfile.write('#36 oldRedshiftFlag [1: Fails to Match That of Other Lines to Within 2.0%, 2: Reviewers Failed To Match Within  2.0%, 4: Reviewers Failed To Match And Neither Was H_alpha, 8: Two lines have same ID, but different Wavelength \n') 
    #outfile.write('#37 oldContaminationFlag [1=no, 2=yes] \n') 
    #outfile.write('#38 oldEdgeFlag [1: Overlaps Upper Limit,  1.675 (um), 2: Overlaps Lower Limit,  0.850 (um), 3: Overlaps Middle Section \n') 
    #outfile.write('#39 oldQualityFlag [+1: Potential Flux Issues, +2: Only multi-line for close, S/N <  5.0, lines (i.e. Ha/SII or Hb/OIII), +4: Single Line Sources, +8: Only Identified By Single Reviewer, +16: Significant Redshift Disagreements \n')

 
   
    for i in np.arange(np.size(redshifts_unique)):
        w1=np.where(beams == beams_unique[i])
        w1 = w1[0][0] 
        z_obj = redshifts[w1] 
        fwhm_obj =fwhm[w1]  
        wave_obj = wavelength[w1] 
        snr_obj = linesnr[w1]
        col3_obj = col3[w1] 
        col4_obj = col4[w1] 
        col5_obj = col5[w1] 
        ...
        if np.size(w1) > 1:  
           w2 = np.where(snr_obj == np.max(snr_obj))
           z_obj = z_obj[w2] 
           fwhm_obj =fwhm_obj[w2]  
           wave_obj = wave_obj[w2] 
           col3_obj = col3_obj[w2] 
           col4_obj = col4_obj[w2]
           col5_obj = col5_obj[w2]
           col6_obj = col6_obj[w2]
           col7_obj = col7_obj[w2]
           col8_obj = col8_obj[w2]
           col9_obj = col9_obj[w2]
        if wave_obj > config['transition_wave']: 
            fwhm_guess  = fwhm_obj / (1+z_obj)    ### (1+z) error in line list? 
        else:  
            fwhm_guess = 2 *  fwhm_obj   / (1+z_obj)   ### make the fwhm_unique a G141 value. 
        outfile_diagnostic.write(str(beams_unique[i]) + '   '  + str(fwhm_guess) + '  ' + str(z_obj) + '\n') 
        print beams_unique[i], fwhm_guess, z_obj
        #### OK DO IT!!! 
        out = fitandplot(dirtospectra, par, beams_unique[i],  z_obj, fwhm_guess, config)  
        #print beams_unique[i], out['hanii_ew_obs'], out['oiii_ew_obs']
 
        ### write it
        outstr =  '{:<8d}'.format(par) + \
                  '{:<6d}'.format(beams_unique[i]) +\
                  '{:<4d}'.format(col3_obj) + \
                  '{:<12.6f}'.format(col4_obj) + \
                  '{:<12.6f}'.format(col5_obj) + \
                  '{:<8.2f}'.format(col6_obj) + \
                  '{:<8.2f}'.format(col7_obj) + \
                  '{:<8.3f}'.format(col8_obj) + \
                  '{:<8.3f}'.format(col9_obj) + \
                  '{:>8.4f}'.format(out['redshift']) + '{:>10.4f}'.format(out['redshift_err']) +\
                  '{:>10.4f}'.format(out['dz_oiii'])  + \
                  '{:>10.4f}'.format(out['dz_oii'])   + \
                  '{:>10.4f}'.format(out['dz_siii_he1']) +\
                  '{:>10.2f}'.format(out['fwhm_g141']) + '{:>10.2f}'.format(out['fwhm_g141_err'])  +  \
                  '{:>13.2e}'.format(out['oii_flux'])  + '{:>13.2e}'.format(out['oii_error']) +  '{:>13.2e}'.format(out['oii_ew_obs']) +\
                  '{:>13.2e}'.format(out['hg_flux']) + '{:>13.2e}'.format(out['hg_error']) + '{:>13.2e}'.format(out['hg_ew_obs']) +\
                  '{:>13.2e}'.format(out['hb_flux']) + '{:>13.2e}'.format(out['hb_error']) + '{:>13.2e}'.format(out['hb_ew_obs']) +\
                  '{:>13.2e}'.format(out['oiii_flux']) + '{:>13.2e}'.format(out['oiii_error']) + '{:>13.2e}'.format(out['oiii_ew_obs']) +\
                  '{:>13.2e}'.format(out['hanii_flux']) + '{:>13.2e}'.format(out['hanii_error']) + '{:>13.2e}'.format(out['hanii_ew_obs']) + \
                  '{:>13.2e}'.format(out['sii_flux']) + '{:>13.2e}'.format(out['sii_error']) + '{:>13.2e}'.format(out['sii_ew_obs']) +\
                  '{:>13.2e}'.format(out['siii_9069_flux']) + '{:>13.2e}'.format(out['siii_9069_error']) + '{:>13.2e}'.format(out['siii_9069_ew_obs']) +\
                  '{:>13.2e}'.format(out['siii_9532_flux']) + '{:>13.2e}'.format(out['siii_9532_error']) + '{:>13.2e}'.format(out['siii_9532_ew_obs']) +\
                  '{:>13.2e}'.format(out['he1_flux']) + '{:>13.2e}'.format(out['he1_error']) +  '{:>12.2e}'.format(out['he1_ew_obs']) +  ' \n' 
        outfile.write(outstr)
         
        ### generate a fig name to merge them in the final step.
        figname =  'fitfigs/fig_beam' + str(beams_unique[i]) + '.pdf '  
        figstring = figstring + figname

    outfile_diagnostic.close()     
    outfile.close()     
    os.system('gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=Par' + str(par)+ '_fitfigs.pdf ' + figstring) 
    print 'finished'
        

def update_object(catalog_to_update, beam, out): 
    ### out = the output of fitandplot, and contains all of the fit results that we want to add to the catalog. 
    tab = asciitable.read(catalog_to_update)
    w=np.where(tab['NUMBER'] == beam) 
    ### I can't get the asciitable to write in a proper sextractor format, so I'm doing it the hard way. 
 
    ### carry these things over from the original catalog
    par = tab['ParID'][w][0] 
    col3_obj = tab['NumberofUserIdentifications'][w][0]
    col4_obj = tab['RA'][w][0] 
    col5_obj = tab['Dec'][w][0] 
    col6_obj = tab['Jmagnitude'][w][0] 
    col7_obj = tab['Hmagnitude'][w][0] 
    col8_obj = tab['A_IMAGE'][w][0] 
    col9_obj = tab['B_IMAGE'][w][0] 

    outfile  = open(catalog_to_update + '.update', 'w')
      
    figstring = ' '
    infile = open(catalog_to_update)
    for line in infile: 
        if line[0] == '#':  
            outfile.write(line) 
        else: 
            ...
            if x[1] == str(beam): 
                outstr =  '{:<8d}'.format(par) + \
                  ...
                  '{:>13.2e}'.format(out['siii_9069_flux']) + '{:>13.2e}'.format(out['siii_9069_error']) + '{:>13.2e}'.format(out['siii_9069_ew_obs']) +\
                  '{:>13.2e}'.format(out['siii_9532_flux']) + '{:>13.2e}'.format(out['siii_9532_error']) + '{:>13.2e}'.format(out['siii_9532_ew_obs']) +\
                  '{:>13.2e}'.format(out['he1_flux']) + '{:>13.2e}'.format(out['he1_error']) +  '{:>13.2e}'.format(out['he1_ew_obs']) +  ' \n' 
                outfile.write(outstr) 
            else: 
               outfile.write(line) 
            figname = 'fitfigs/fig_beam' + x[1] + '.pdf '
            figstring = figstring + figname 
    ...
    infile.close()
    outfile.close()  
    os.rename(catalog_to_update + '.update', catalog_to_update)
    os.system('gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=Par' + str(par)+ '_fitfigs.pdf ' + figstring)
